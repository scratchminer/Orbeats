...
function Note:update(currentBeat, orbitRadius)
  ...
  if not self.hitting then
    if self.finishBeat == nil then
      self.radius = ((orbitRadius - initialRadius) / (math.exp(self.spd * self.lifeLength) - 1)) *
      (math.exp(self.spd * beatsSinceSpawn) - 1) + initialRadius
    else
      self.radius = ((orbitRadius - initialRadius) / (math.exp(self.spd * self.lifeLength) - 1)) *
      (math.exp(self.spd * (beatsSinceSpawn - (self.finishBeat - self.hitBeat))) - 1) + initialRadius
      -- print(beatsSinceSpawn-(self.finishBeat-self.hitBeat))
    end
  end

  if self.duration > 0 then
    self.endRadius = ((orbitRadius - initialRadius) / (math.exp(self.spd * self.lifeLength) - 1)) *
    (math.exp(self.spd * (beatsSinceSpawn - self.duration)) - 1) + initialRadius
  else
  ...
  self.currentPos = ((self.hitPos - spinPos) / (self.hitBeat - self.spawnBeat)) * (currentBeat - self.spawnBeat) +
  spinPos
  ...
  return { oldRadius = oldRadius, newRadius = self.radius, position = self.currentPos, noteType = "note", endRadius =
  self.endRadius, hitting = self.hitting, endBeat = self.duration + self.hitBeat, hitBeat = self.hitBeat }
end
